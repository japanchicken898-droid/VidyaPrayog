...
export default function CodingSandboxModal({ isOpen, onClose, onSubmitScore }) {
  ...
  const executeCode = () => {
    ...
    setTimeout(() => {
      ...
      setTestResults([
        { id: 1, passed: passed, input: "Example 1", output: passed ? "Correct" : "Wrong Answer" },
        { id: 2, passed: true, input: "Example 2", output: "Correct" }
      ]);
  ...
  const handleFinalSubmit = () => {
    if (onSubmitScore) {
      onSubmitScore({ score: 85, badgeUnlocked: true });
    }
    onClose();
  };
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Updated CodingSandboxModal.jsx to strict IDE layout")