...
import { Search, Bell, ArrowLeft, Download, Filter } from 'lucide-react';
    ...
    <header className="docked full-width top-0 h-20 bg-white/80 border-b border-slate-200/60 fixed top-0 right-0 left-0 md:left-72 z-30 flex justify-between items-center px-6 md:px-10 backdrop-blur-md">
      <div className="flex items-center gap-4 flex-1 relative">
        ...
        <div className="relative w-full max-w-xs hidden sm:block">
          ...
          <input 
            ...
            className="w-full bg-slate-50/80 border border-slate-200/80 rounded-full py-2 pl-11 pr-4 text-xs font-semibold focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all outline-none text-slate-800" 
            placeholder="Search student progress, projects, partners..." 
            type="text"
          />
        </div>

        {/* Department Filter dropdown */}
        <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-full px-3 py-1.5 text-xs font-bold text-slate-600">
          <Filter className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-[10px] uppercase text-slate-400 mr-1">Dept:</span>
          <select 
            value={deptFilter}
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <button 
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full text-xs font-extrabold shadow-md shadow-indigo-500/10 active:scale-95 transition-all flex items-center gap-1.5"
        >
        ...
        <div className="relative">
          ...
          {notifMenuOpen && (
            <div className="absolute right-0 top-12 w-80 bg-white border border-slate-200/80 rounded-2xl shadow-xl p-4 z-50 animate-fade-in text-left">
              <h4 className="font-extrabold text-xs text-slate-900 mb-3 pb-2 border-b">Governance Alerts</h4>
                  ...
                  notifications.map((notif) => (
                    ...
                    >
                      <span className="material-symbols-outlined text-indigo-600 text-sm mt-0.5">
                        {notif.type === 'placement' ? 'work_outline' : 'corporate_fare'}
                      </span>
                      <div>
