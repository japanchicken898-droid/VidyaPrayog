...
const TRACKS = [
  { id: 1, title: 'Python Basics', solvedCount: 12, progress: 13, icon: (
    <svg viewBox="0 0 110 110" className="w-7 h-7">
      <path fill="#387EB8" d="M53.8,11.2c-21.6,0-20.5,9.4-20.5,9.4l0.1,9.8h21.1v3H31.6c0,0-14.8-1.7-14.8,19.9 c0,21.6,12.7,21.3,12.7,21.3h5.9v-10.1c0,0,-0.2,-12,12.2,-12h20.8c0,0,11.6,0.3,11.6,-11.3V19.9C80,19.9,81.4,11.2,53.8,11.2z M43.4,20.4c1.9,0,3.5,1.6,3.5,3.6c0,1.9-1.6,3.5-3.5,3.5c-1.9,0-3.5-1.6-3.5-3.5C39.9,22,41.5,20.4,43.4,20.4z"/>
      <path fill="#FFE052" d="M55.8,98.8c21.6,0,20.5-9.4,20.5-9.4l-0.1-9.8H55.1v-3h22.9c0,0,14.8,1.7,14.8-19.9 c0-21.6-12.7-21.3-12.7-21.3h-5.9v10.1c0,0,0.2,12-12.2,12H41.2c0,0-11.6-0.3-11.6,11.3v21.3C29.6,90.1,28.2,98.8,55.8,98.8z M66.2,89.6c-1.9,0-3.5-1.6-3.5-3.6c0-1.9,1.6-3.5,3.5-3.5c1.9,0,3.5,1.6,3.5,3.5C69.7,88,68.1,89.6,66.2,89.6z"/>
    </svg>
  )},
  { id: 2, title: 'Data Structures in C++', solvedCount: 0, progress: 0, icon: (
    <div className="relative flex items-center justify-center w-7 h-7">
      <svg viewBox="0 0 24 24" className="absolute inset-0 w-full h-full text-[#00599C] fill-current">
        <polygon points="12 2 22 7.8 22 16.2 12 22 2 16.2 2 7.8" />
      </svg>
      <span className="relative z-10 text-white font-bold text-[8px] tracking-tighter">C++</span>
    </div>
  )},
];

export default function CodingSandboxModal({ isOpen, onClose }) {
      ...
      <div className="bg-[#f8fafc] dark:bg-[#090d16] rounded-3xl w-full max-w-[95vw] lg:max-w-7xl shadow-2xl border border-white/20 dark:border-slate-800/80 overflow-hidden flex flex-col h-[95vh] relative text-slate-900 dark:text-slate-200 transition-colors">
        ...
        <div className="px-6 py-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-sm font-black tracking-tight text-slate-700">{view === 'explorer' ? 'Coding Practice Portal' : activeProblem?.title}</h2>
          </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          <AnimatePresence mode="wait">
            {view === 'explorer' ? (
              <ExplorerView key="explorer" onSolve={handleSolve} />
            ) : (
