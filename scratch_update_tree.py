...
import { FULLSTACK_ROADMAP, BACKEND_ROADMAP } from "../../data/roadmapData";
...
const RoadmapTree = ({ studentProfile, onOpenCoding }) => {
  const role = studentProfile?.targetRole || "";
  const track = ROLE_TO_TRACK[role] || "fullstack";
  
  const roadmapData = track === "backend" ? BACKEND_ROADMAP : FULLSTACK_ROADMAP;
    ...
    <div className="w-full flex flex-col items-center pb-20 relative">
      ...
      <div className="w-full max-w-3xl mb-12 bg-white border-2 border-slate-900 p-4 rounded-2xl shadow-[4px_4px_0px_rgba(15,23,42,1)] sticky top-0 z-40">
        <div className="flex justify-between items-center mb-2">
          <span className="font-black text-slate-800 tracking-wide uppercase text-sm">Roadmap Progress</span>
