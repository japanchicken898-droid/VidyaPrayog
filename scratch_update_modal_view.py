...
# Update Modal Header
text = text.replace('<h2 className="text-lg font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">Coding Practice Portal</h2>', '<h2 className="text-lg font-black text-slate-800 dark:text-slate-100 flex items-center gap-2">Proctored Coding Benchmark</h2>')
text = text.replace("Coding Practice Portal", "Proctored Coding Benchmark")

# Completely replace ExplorerView inside CodingSandboxModal
new_explorer = """function ExplorerView({ onSolve }) {
  // 5 formal benchmark problems
...
# Replace the old ExplorerView function entirely
text = re.sub(r'function ExplorerView\(\{ onSolve \}\) \{.*?(?=function EditorView)', new_explorer + '\n\n', text, flags=re.DOTALL)

with open(path, 'w', encoding='utf-8') as f:
    f.write(text)
print("Updated CodingSandboxModal.jsx ExplorerView")